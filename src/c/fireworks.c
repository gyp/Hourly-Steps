...
void Firework_Start(FireworkStruct* firework) {
  ...
  sll xSpeedVal = sllsub(rand_sll(CONST_4), int2sll(2));
...
void Firework_Move(FireworkStruct* firework) {
  ...
  if (firework->ySpeed[0] > CONST_0) {
    for (int loop = 0; loop < FIREWORK_PARTICLES; loop++) {
      ...
      firework->xSpeed[loop] = sllsub(rand_sll(CONST_8), int2sll(4));
      firework->ySpeed[loop] = sllsub(rand_sll(CONST_8), int2sll(4));
    }
...
void Firework_Initialize(int width, int height) {
  ...
  Firework_baselineYSpeed = slldiv(sllneg(int2sll(25)), CONST_10);
